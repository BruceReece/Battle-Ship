...
public class Board 
{
    ...
    private List<Ship> shipList;
       private int index,hitCount,missCount,
            checkSide1,checkSide2,checkSide3,checkSide4,tryy;
     ...
     *                  passed to placedOnBoard() and setCoordinates()
     ...
     * preconditions:  a ship has been placed on the board
  ...
  *               based on orientation;repeat placedOnBoard() until successful
  * preconditions:  Ships[] are constructed
  * postconditions: Ships[] are arranged onto the board via random
  * Calls: placedOnBoard()
  * @param ships 
  */   
    ...
    {
       String board1 = " Galina & Bruce SeaBoard\n"; // string representing board
    ...
    public String compBoard()
    {
       ...
       for (int i = 0; i < SIZE; i++, y++) 
       {
           ...
           for (int j = 0; j < SIZE; j++)
           {
               ...
               {
                   cBoard = cBoard.concat(board[i][j] + "\t");
               }
    ...
    /**
     * Mutator: checkFor user's guess is a hit of a ship
     * Updates board with M for a Miss, beeps and updates board with H for hit
     * @param x
     * @param y
    ...
    public boolean checkForHit(int x, int y)
    {
        ...
        if (whatsThere != '^' && whatsThere != 'S' && 
                whatsThere != 'H' && whatsThere != 'M')
        { // must be a ship if not one of these
    ...
    public void tryMoves(int x, int y)
    {
        ...
        switch(tryy)
        {
              ...
              case 0: checkSide1 = x-index;
                    whatsThere = getSymbol(checkSide1 ,y) ; 
                    if(checkSide1 < 0 || checkSide1 > SIZE)
                    {
                        tryy =2;
                        tryMoves(x,y);                      
                    }
                    else if ( whatsThere == 'S' || whatsThere == 'M')
                        {
                            tryy =2;
                            tryMoves(x,y);
                        }
                       
                    if (checkForHit(checkSide1,y)==true)
                    {
                       index++; 
                    }
                    else 
            ...
            case 1: checkSide2 = y-index;
                    whatsThere = getSymbol(x,checkSide2) ; 
                    if(checkSide2 < 0 || checkSide2 > SIZE)
                    {
                        tryy =0;
                        stillHit = false;
                    }
                    ...
                    if (checkForHit(x, checkSide2)==true)
                    {
                       index++; 
                    }
                    else 
            ...
            case 2: checkSide3 = x+index;
                    whatsThere = getSymbol(checkSide3 ,y) ; 
                    if(checkSide3 < 0 || checkSide3 > SIZE)
                    {
                        tryy =3;
                       tryMoves(x,y);
                    }
                    ...
                    if (checkForHit(checkSide3,y)==true)
                    {
                       index++; 
                    }
                    else 
            ...
            case 3: checkSide4 = y+index;
                    whatsThere = getSymbol(x,checkSide4) ; 
                    if(checkSide4 < 0 || checkSide4 > SIZE)
                    {
                        tryy =1;
                       tryMoves(x,y);
                    }
                    ...
                    if (checkForHit(x, checkSide4)==true)
                    {
                       index++; 
                    }
                    else 
